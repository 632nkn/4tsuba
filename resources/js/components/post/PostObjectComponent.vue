<template>
    <v-card v-if="post.deleted_at === null" color="light-green lighten-5" outlined class="my-3">
        <div>
            <!-- １行目 -->
                ...
                ></v-checkbox>
                    <v-tooltip bottom>
                        <template v-slot:activator="{ on }">
                            <span v-on="on"
                                ><v-icon class="ml-3 mt-n2">mdi-message-arrow-left-outline</v-icon></span
                            >
                        </template>
                        <span>返信</span>
                    </v-tooltip>
                    <template v-if="post.user_id === user_id">
                    <v-tooltip bottom>
                        <template v-slot:activator="{ on }">
                            <span v-on="on"
                                ><v-icon class="ml-3 mt-n2" @click="editPost">mdi-lead-pencil</v-icon></span
                            >
                        </template>
                        <span>編集</span>
                    </v-tooltip>
                    <v-tooltip bottom>
                        <template v-slot:activator="{ on }">
                            <span v-on="on"
                                ><v-icon class="ml-3 mt-n2" @click="deleteAndEmit">mdi-delete</v-icon></span
                            >
                        </template>
                        <span>削除</span>
                    </v-tooltip>
                    ...
                    <template v-if="post.responded_count">
                        ...
                        <router-link
                            v-bind:to="{
                                ...
                                params: { thread_id: post.thread_id, displayed_post_id: post.displayed_post_id }
                            }"
                        >
...
export default {
    ...
    methods: {
        ...
        deleteAndEmit() {
            console.log("this is deleteAndEmit");
